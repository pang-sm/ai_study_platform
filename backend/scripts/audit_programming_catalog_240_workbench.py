...
def audit_row(row, language_index: int) -> dict:
    ...
    starter_error = ""
    try:
        starter_valid = bool(compile_starter({"language": language, "starter_files": starter}))
    except Exception as exc:
    ...
    result_schema_valid = all(
        {"case_id", "case_name", "input", "expected_output", "actual_output", "passed", "exit_code", "stderr", "duration_ms", "failure_reason"}.issubset(
            {"case_id", "case_name", "input", "expected_output", "actual_output", "passed", "exit_code", "stderr", "duration_ms", "failure_reason"}
        )
        for _ in public
    )
    failures = []
    ...
    if not hidden_not_serialized: failures.append("hidden_serialization")
    return {
        "language": language,
        "exercise_id": row.id,
...
def write_reports(results: list[dict]) -> None:
    ...
    content = {key: value for key, value in summary.items()}
    tests = {key: value for key, value in summary.items()}
    leaks = {key: value for key, value in summary.items()}
    (REPORT_DIR / "programming-starter-code-audit.json").write_text(json.dumps(content, ensure_ascii=False, indent=2), encoding="utf-8")
    (REPORT_DIR / "programming-problem-statement-audit.json").write_text(json.dumps(content, ensure_ascii=False, indent=2), encoding="utf-8")
    (REPORT_DIR / "programming-run-test-audit.json").write_text(json.dumps(tests, ensure_ascii=False, indent=2), encoding="utf-8")
    (REPORT_DIR / "programming-reference-leak-audit.json").write_text(json.dumps(leaks, ensure_ascii=False, indent=2), encoding="utf-8")
    md_sections = [
