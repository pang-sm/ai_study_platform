...
function App() {
        ...
        <div className="auth-center-panel">
            ...
            {isLogin ? (
              <div className="auth-form">
                {loginTab === "password" ? (
                  <>
                    <label className="auth-input-wrap"><span className="auth-input-icon">U</span><input className="auth-input" placeholder={"\u8d26\u53f7"} aria-label={"\u8d26\u53f7"} value={username} onChange={(e) => setUsername(e.target.value)} /></label>
  ...
  // ── Independent 11408 AI question attempt page ──
  const aiAttemptMatch = window.location.pathname.match(/^\/exam\/11408\/([a-z_]+)\/ai-questions\/attempt\/(\d+)/);
  if (aiAttemptMatch) {
    const aiSubjectKey = aiAttemptMatch[1];
    const aiAttemptId = parseInt(aiAttemptMatch[2], 10);
    return (
      <div className="app-shell">
        <Suspense fallback={<div className="empty-state">加载中...</div>}>
