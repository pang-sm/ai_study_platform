...
function RecordDetail({ record }: { record: Record }) {
  // Only targets that can be addressed by a canonical id the record actually carries are
  // linked. A past-paper question's identity IS (module, year, question_number), so the
  // question number travels; a chapter-practice record carries no chapter or attempt id,
  // so it links to its module's practice rather than inventing a chapter from a title.
  const pastPaper = record.source_kind === 'past_paper' && record.year !== null;
