...
export default function ExamAIQuestionAttemptPage({ subjectKey, attemptId, user, onBack }) {
  ...
  useEffect(() => {
    ...
    safeFetch(`${API_BASE}/exam/11408/${subjectKey}/ai-questions/attempts/${attemptId}?username=${user?.username||""}`)
      .then(d => { setAttempt(d.attempt); setQuestions(d.questions||[]); if(d.saved_answers)setAnswers(d.saved_answers); if(d.attempt?.status==="submitted"){loadResult(d.saved_answers||{});} })
      .catch(e => setError(e.message)).finally(() => setLoading(false));
  }, [attemptId]);

  const loadResult = async (savedAnswers) => {
    try {
      const r = await safeFetch(`${API_BASE}/exam/11408/${subjectKey}/ai-questions/attempts/${attemptId}/submit`, {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:user?.username,answers:savedAnswers||answers})});
  ...
  // Result view — matching chapter practice experience
  if(submitted&&result){
    ...
    return (<div className="attempt-page">
      <header className="attempt-page-header">
        <div className="attempt-page-header-left">
          ...
          <div><h2>AI 出题练习 · 结果</h2><p>{attempt?.knowledge_point_path||"综合出题"} · {result.total_questions} 题</p></div>
        </div>
      </header>
      <div className="attempt-page-body">
        <div className="attempt-page-result">
          ...
          <div className="past-paper-result-actions" style={{marginBottom:16}}>
            <button className="ghost-button compact" onClick={onBack}>返回 AI 题库</button>
          </div>
  ...
  // Practice view — with question navigation like chapter practice
  return (<div className="attempt-page">
    <header className="attempt-page-header">
      <div className="attempt-page-header-left">
        <button className="ghost-button compact" onClick={()=>{window.close();setTimeout(()=>{if(!window.closed)setCloseHint("请手动关闭当前标签页。")},300);}}>← 关闭</button>
        <div><h2>AI 出题练习</h2><p>{attempt?.knowledge_point_path||"综合出题"} · {questions.length} 题</p></div>
      </div>
