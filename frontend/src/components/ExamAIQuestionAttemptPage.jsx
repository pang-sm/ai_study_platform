...
export default function ExamAIQuestionAttemptPage({ subjectKey, attemptId, user, onBack }) {
  ...
  return (<div className="attempt-page">
    ...
    {submitted&&result?(<div className="attempt-page-result"><h3>练习结果</h3>
      <div className="practice-stats-grid" style={{marginBottom:16}}>
        ...
        <div className="practice-stat-card--dashboard"><div className="practice-stat-card-icon practice-stat-card-icon--done">✓</div><div className="practice-stat-card-body"><div className="practice-stat-card-value">{result.correct}/{result.total_questions}</div><div className="practice-stat-card-label">正确数</div></div></div>
      ...
      <div className="past-paper-result-actions"><button className="primary-button compact" onClick={()=>{setAnswers({});setSubmitted(false);setResult(null);}}>重新练习</button><button className="ghost-button compact" onClick={onBack}>返回AI题库</button></div>
      {result.results&&result.results.map(r=>{const q=questions.find(q=>q.id===r.question_id);return(<div key={r.question_id} className={`past-paper-question-card ${r.correct?"past-paper-q-correct":"past-paper-q-wrong"}`}><div className="past-paper-question-meta"><span className="past-paper-q-number">第 {questions.indexOf(q)+1} 题</span><span className="past-paper-q-type">选择题</span><span className={`past-paper-q-result-tag ${r.correct?"past-paper-q-result-tag--correct":"past-paper-q-result-tag--wrong"}`}>{r.correct?"正确":"错误"}</span></div>{q?.stem&&<div className="past-paper-q-content">{q.stem}</div>}<div className="past-paper-q-answer-row"><span className="past-paper-q-label">你的答案：</span><span className={r.correct?"text-correct":"text-wrong"}>{r.user_answer}</span></div><div className="past-paper-q-answer-row"><span className="past-paper-q-label">标准答案：</span><span>{r.standard_answer}</span></div>{q?.analysis&&<div className="past-paper-q-feedback"><strong>解析</strong><p>{q.analysis}</p></div>}</div>)})}
