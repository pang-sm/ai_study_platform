...
const pdf = fs.readdirSync(dir).filter(f => f.startsWith("regression-course-learning-unique") && f.endsWith(".pdf")).map(f => path.join(dir, f))[0];
...
await page.locator("input[type=file]").first().setInputFiles(pdf).catch((e) => console.log("upload err", e.message));
...
console.log("visible files after upload:", JSON.stringify(files));
